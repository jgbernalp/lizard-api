...
class CRUDController extends BaseController {
  ...
  read(req, res, next) {
    if (req.params.id) {
      this.model.findById({ id: req.params.id, fields: req.criteria.fields, options: { lean: true } })
        .then(resource => {
          if (resource == null) {
  ...
  create(req, res, next) {
    this.model.validate(req.body).then(({ isValid, validationErrors }) => {
      if (!isValid) {
  ...
  update(req, res, next) {
    ...
    this.model.findById({ id: req.params.id }).then(resource => {
      if (resource == null) {
      ...
      this.model.validate(resource).then(({ isValid, validationErrors }) => {
        if (!isValid) {
          return next(new HTTPError(400, 'Invalid data', validationErrors));
        } else {
  ...
  delete(req, res, next) {
    ...
    this.model.findOne({ query: { _id: req.params.id } }).then(resource => {
      if (resource == null) {
        return next(new HTTPError(404));
      }
